import type { Category, DayInfo, ParentCategory } from '../types';
...
export const ICONS: Record<string, string> = {
  ...
  move:'<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M8 12h8"/><path d="M12 8l4 4-4 4"/></svg>',
...
export const PARENT_CATEGORIES: Record<ParentCategory, Category & { subTypes?: Record<string, Category> }> = {
  visit:   { cls: 'v',  lbl: '訪問',      ico: 'building' },
  atxsg:   { cls: 'ev', lbl: 'ATxSG',     ico: 'broadcast' },
  move:    { cls: 'mv', lbl: '移動',      ico: 'move', subTypes: {
    taxi:   { cls: 'tx', lbl: 'タクシー',  ico: 'car' },
    mrt:    { cls: 'mr', lbl: 'MRT',       ico: 'train' },
    flight: { cls: 'fl', lbl: '飛行機',    ico: 'plane' },
    walk:   { cls: 'wk', lbl: '徒歩',     ico: 'walk' },
  }},
  food:    { cls: 'fd', lbl: '食事',      ico: 'utensils', subTypes: {
    lunch:  { cls: 'lu', lbl: 'ランチ',    ico: 'utensils' },
...
export const PARENT_CATEGORY_KEYS: ParentCategory[] = ['visit', 'atxsg', 'move', 'food', 'sync', 'review', 'reserve'];

/** Get display info for a block's category + subType */
export function getCatDisplay(category: string, subType?: string): Category {
  const parent = PARENT_CATEGORIES[category as ParentCategory];
  if (!parent) return { cls: 'rv', lbl: category || '不明', ico: 'clock' };
  if (subType && parent.subTypes && parent.subTypes[subType]) {
    return parent.subTypes[subType];
  }
  return parent;
}

/** Get CSS class for block */
export function getCatCls(category: string, subType?: string): string {
  const parent = PARENT_CATEGORIES[category as ParentCategory];
  if (!parent) return 'rv';
  if (subType && parent.subTypes && parent.subTypes[subType]) {
    return parent.subTypes[subType].cls;
  }
  return parent.cls;
}

/** Legacy type → new category + subType */
export function legacyToCategory(type: string): { category: ParentCategory; subType: string } {
  switch (type) {
    case 'visit': return { category: 'visit', subType: '' };
    case 'event': return { category: 'atxsg', subType: '' };
    case 'taxi': return { category: 'move', subType: 'taxi' };
    case 'mrt': return { category: 'move', subType: 'mrt' };
    case 'flight': return { category: 'move', subType: 'flight' };
    case 'walk': return { category: 'move', subType: 'walk' };
    case 'hotel_move': return { category: 'move', subType: 'taxi' };
    case 'travel': return { category: 'move', subType: 'taxi' };
    case 'lunch': return { category: 'food', subType: 'lunch' };
    case 'dinner': return { category: 'food', subType: 'dinner' };
    case 'sync': return { category: 'sync', subType: '' };
    case 'review': return { category: 'review', subType: '' };
    case 'reserve': return { category: 'reserve', subType: '' };
    default: return { category: 'reserve', subType: '' };
  }
}
