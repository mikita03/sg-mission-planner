...
export function BootScreen({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    if (started.current) return; // prevent double-run
...
export function addTrail(x: number, y: number, color = '0,229,255') { _trails.push({ x, y, a: 0.5, color }); }
export function addRipple(x: number, y: number, color = '0,229,255') { _ripples.push({ x, y, r: 0, a: 0.6, color }); }
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (let i = _trails.length - 1; i >= 0; i--) {
        ...
        ctx!.shadowColor = `rgba(${t.color},${t.a})`; ctx!.shadowBlur = 8; ctx!.fill(); ctx!.shadowBlur = 0;
        t.a -= 0.015; if (t.a <= 0) _trails.splice(i, 1);
      }
      for (let i = _ripples.length - 1; i >= 0; i--) {
        const rp = _ripples[i];
        ctx!.beginPath(); ctx!.arc(rp.x, rp.y, rp.r, 0, Math.PI * 2);
        ctx!.strokeStyle = `rgba(${rp.color},${rp.a})`; ctx!.lineWidth = 2; ctx!.stroke();
        if (rp.r > 10) { ctx!.beginPath(); ctx!.arc(rp.x, rp.y, rp.r * 0.6, 0, Math.PI * 2); ctx!.strokeStyle = `rgba(${rp.color},${rp.a * 0.5})`; ctx!.lineWidth = 1; ctx!.stroke(); }
        if (rp.a > 0.2) { for (let k = 0; k < 6; k++) { const angle = (k / 6) * Math.PI * 2 + rp.r * 0.05; ctx!.beginPath(); ctx!.arc(rp.x + Math.cos(angle) * rp.r, rp.y + Math.sin(angle) * rp.r, 1.5, 0, Math.PI * 2); ctx!.fillStyle = `rgba(${rp.color},${rp.a})`; ctx!.fill(); } }
        rp.r += 2.5; rp.a -= 0.015; if (rp.a <= 0) _ripples.splice(i, 1);
      }
      animId = requestAnimationFrame(draw);
    }
    draw();
...
export function MouseGlow() {
  ...
  useEffect(() => {
    function onMove(e: MouseEvent) { if (ref.current) { ref.current.style.left = e.clientX + 'px'; ref.current.style.top = e.clientY + 'px'; ref.current.classList.add('visible'); } }
...
export function triggerGlitch() {
  const el = document.createElement('div'); el.className = 'glitch-band'; document.body.appendChild(el); setTimeout(() => el.remove(), 500);
}

/* ═══ Toast ═══ */
let _toastTimer = 0;
export function showToast(msg: string) {
  const el = document.getElementById('globalToast'); if (!el) return;
  el.textContent = msg; el.classList.add('show');
  clearTimeout(_toastTimer); _toastTimer = window.setTimeout(() => el.classList.remove('show'), 2000);
}
export function Toast() { return <div className="tt" id="globalToast" />; }
